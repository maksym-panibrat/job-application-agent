...
Pre-decomposes <script> and <style> elements with BeautifulSoup so
their inner text is dropped entirely (markdownify's `strip=` parameter
removes only the tag wrapper, not the body — JS/CSS source would leak).
Then runs markdownify (ATX headings) and collapses runs of >2 blank lines.
